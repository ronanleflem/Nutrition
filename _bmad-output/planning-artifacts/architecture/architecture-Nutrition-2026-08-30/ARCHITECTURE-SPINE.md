...
- **Rule:** `Product.preferredReferenceId` pointe la source des macros pour 100 g ; calcul recettes/plan lit cette référence ; si absente, UI bloque ou force le choix d’une référence préférée
...
Index Dexie clés : `productReferences.productId`, `productReferences.barcode`, `productReferences.nutritionalScore`, `productReferences.store`.
...
| Recettes + macros | `features/recipes` + `core/scoring` | AD-4, AD-5, AD-11 |
| Objectifs + synthèse | `features/macro-goals` + `features/meal-plan` | AD-5 |
| Plan de repas | `features/meal-plan` | AD-4 |
